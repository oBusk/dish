...
echo "Sep=;"
echo "DNS Ip; hostname; DNS Organization; whoami 'ns'; ns location; ns distance; whoami 'ecs'; whoami 'ip'; google.com average latency; Akamai Edge; Akamai Hostname; Akamai Edge Location; Akamai Edge Distance; Akamai Edge Pretransfer; Netflix Edge; Netflix Hostname; Netflix Edge Location; Netflix Edge Distance; Netflix Edge Pretransfer; Youtube Edge; Youtube Hostname; Youtube Edge Location; Youtube Edge Distance; Youtube Edge Pretransfer; CDN Average Pretransfer"

echo "$INPUT" | parallel -j 8 source benchmark.sh {}
